...
- **閱讀** —— 文獻有合法開放存取連結的話,Claude 可以直接抓全文來讀,回答摘要答不出來的問題,或跨多篇文獻做整合統整。
...
目錄裡永遠會存**書目 metadata**(標題/作者/年份/期刊/出版社/領域/DOI/URL——這些不受著作權保護)以及有提供時的簡短**摘要**。除此之外,真正的界線是「有沒有合法存取權限」,不是「metadata vs. 全文」:

- OpenAlex 對大多數文獻都會告訴我們有沒有合法開放存取版本、在哪裡——這個**連結**(`oa_url`)會存進條目,匯出 BibTeX 時也會附上,而且 Claude 可以直接讀那個連結的內容來幫你整理重點或統整——跟你自己用瀏覽器打開沒有本質差異。
- 沒有 `oa_url` 的話,這個工具不會去找繞過去的辦法——不爬付費牆、不找非官方鏡像。如果你自己本來就有合法管道(機構訂閱下載、自己買的 PDF),直接把內容或檔案交給 Claude,一樣可以幫你讀、幫你整理——這是你在用自己本來就有的存取權限。
